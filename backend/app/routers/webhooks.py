...
from app.models import Call, Negotiation
...
async def happyrobot_webhook(
    ...
    if not call:
        ...
        if not mc_number:
            raise HTTPException(
                status_code=400,
                detail="MC number required to create call record"
            )
        ...
        call = Call(
            ...
            phone_number=webhook_data.metadata.get("phone_number") if webhook_data.metadata else None,
        )
        db.add(call)
        db.commit()
        db.refresh(call)
    ...
    if webhook_data.extracted_data:
        ...
        update_data["negotiation_rounds"] = webhook_data.extracted_data.get("negotiation_rounds", 0)
    
    if webhook_data.metadata:
        if "ended_at" in webhook_data.metadata:
            update_data["ended_at"] = datetime.fromisoformat(webhook_data.metadata["ended_at"])
...
def create_call(
    ...
    """Create a new call record (for testing or manual entry)."""
